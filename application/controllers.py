...
def home():
    ...
    return render_template('index.html', is_landing_page=True , products=products, categories=categories)
...
def customer(id):
    if g.user:
        ...
        for product in products:
            ...
            if product.category not in categories:
                categories.append(product.category)
        return render_template('index.html', is_landing_page=False, categories=categories, user=user, products=products)
...
def register():
    if request.method == "GET":
        ...
        if len(users) == 1:
            ...
            user = User(name=name, username=username, age=age, role=role,
                        city=city, password=password, mobile_no=mobile_no, email=email)
            db.session.add(user)
            db.session.commit()
            users = User.query.filter_by(username=username).all()
            if g.user and session['role'] == 'admin':
